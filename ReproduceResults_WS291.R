...
intestine_genes <- c(
  "WBGene00006926", "WBGene00006925", "WBGene00006927", "WBGene00006928",
...
SPERM.SPECIFIC.GENES <- setdiff(SPERM.SPECIFIC.GENES, c(
  "WBGene00004975", "WBGene00021771", "WBGene00004960", "WBGene00007732", "WBGene00004959",
...
SPERM.ASSOCIATED.GENES <- union(
  row.names(genes_sperm[rowMaxs(genes_sperm) > 0.99 & !is.na(rowSums(genes_sperm)), ]),
...
spike_mito_msp_rRNA <- function(ct, cell_name) {
  c(
    "mito_rate" = as.numeric(sum(ct[MITO_GENES, cell_name]) / colSums(ct[-c(1:92), ])[cell_name]),
...
spike_mito_msp_rRNA_stat <- do.call(rbind, lapply(row.names(worms.mapping.info), FUN = function(x) {
  spike_mito_msp_rRNA(salmon_quant$gene$abundance, x)
}))
...
worms.mapping.info$read.type <- "se"
worms.mapping.info$Batch.ID <- "Tintori_2016"
...
worms.mapping.info$tissue <- c(
  "S1" = "Mitotic Region and Transition Zone",
  "S2" = "Pachytene Zone",
  "S3" = "Diplotene Loop",
  "S4" = "Diakinesis",
  "F3" = "-3 Oocyte",
...
# Genes Found
genes_by_type <- tapply(
  row.names(pData(ce.ct)),
...
tzur_genes_by_type <- tapply(colnames(tzur_2018), tzur_2_su, function(x) {
  row.names(tzur_2018)[rowMeans(tzur_2018[, x]) > 1]
})
...
oet_2014 <- data.frame(do.call(cbind, lapply(colnames(oet_2014), function(n) {
  tapply(oet_2014[, n], oet_2014$GENEWBID, function(x) {
...
diag_2018_meta$replicate <- sapply(strsplit(row.names(diag_2018_meta), split = "_"), function(x) {
  x[4]
})
row.names(diag_2018_meta) <- paste(sapply(strsplit(row.names(diag_2018_meta), split = "_"), function(x) {
  x[4]
}), diag_2018_meta$X2, sep = "_")
...
diag_2018_meta$norm_pos <- do.call(c, tapply(row.names(diag_2018_meta),
  ...
  FUN = function(x) {
    min_x1 <- min(as.numeric(diag_2018_meta[x, "X1"]))
    max_x1 <- max(as.numeric(diag_2018_meta[x, "X1"]))
...
diag_genes_by_type <- tapply(colnames(diag_2018), diag_2018_meta$su_label, function(x) {
  row.names(diag_2018)[rowMeans(diag_2018[, x]) > 1]
})
...
comparePriorStudies <- function(dataset1, dataset2, prior_data_name) {
  ...
  for (n in names(dataset2)) {
    ...
    plt <- ggvenn(
      show_percentage = T, auto_scale = T,
      x,
...
sapply(names(oet_2014_genes), FUN = function(x) {
  c(length(intersect(genes_by_type[[x]], oet_2014_genes[[x]])), length(oet_2014_genes[[x]]))
})
sapply(names(diag_genes_by_type), FUN = function(x) {
  c(length(intersect(genes_by_type[[x]], diag_genes_by_type[[x]])), length(diag_genes_by_type[[x]]))
})
sapply(names(tzur_genes_by_type), FUN = function(x) {
  c(length(intersect(genes_by_type[[x]], tzur_genes_by_type[[x]])), length(tzur_genes_by_type[[x]]))
})
sapply(names(liteseq_2018_genes), FUN = function(x) {
  c(length(intersect(genes_by_type[[x]], liteseq_2018_genes[[x]])), length(liteseq_2018_genes[[x]]))
})
...
worms.genes.id <- WORM.GENES[row.names(filtered_hisat), c(1, 2, 4, 7, 16)]
worms.genes.id$gene_short_name <- worms.genes.id$Public.Name
...
num_summary <- data.frame(
  num_of_Genes = colSums(exprs(ce.ct) > 0),
  Batch = ce.ct$experiment,
  cellType = c(ce.ct$cellType)
) %>%
...
num_summary %>%
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 7), legend.key.size = unit(0.4, "cm"))
...
ggplot(logfc_in_other_comps, mapping = aes(x = Log2FC, fill = comparison)) +
...
filter_genes_single_table <- rbind(
  data.frame("Gene Collection" = rep("Sperm Specific Genes", length(SPERM.SPECIFIC.GENES)), WORM.GENES[SPERM.SPECIFIC.GENES, c(1, 2)]),
...
bar_test <- dot_test$data[, c("ID", "Description", "lengthened", "shortened")] %>%
  melt(id.vars = c("ID", "Description"), variable.name = "change") %>%
  ggplot(aes(fill = change, y = value, x = Description)) +
  coord_flip() +
...
bar_test <- dap_summary %>%
  melt(id.vars = c("Comparison"), variable.name = "change") %>%
...
rmats_summary <- do.call(cbind, lapply(rmats_diff_res, FUN = function(x) {
...
rmats_ora_S1vS2 <- enrich_CP(do.call(c, lapply(rmats_diff_res$S1_v_S2, function(x) {
  return(x$GeneID)
})), "celegans", universe = do.call(c, rmats_diff_res$S1_v_S2$universe))
...
rmats_ora_S2vS3 <- enrich_CP(do.call(c, lapply(rmats_diff_res$S2_v_S3, function(x) {
  return(x$GeneID)
})), "celegans", universe = do.call(c, rmats_diff_res$S2_v_S3$universe))
...
rmats_ora_S3vS4 <- enrich_CP(do.call(c, lapply(rmats_diff_res$S3_v_S4, function(x) {
  return(x$GeneID)
})), "celegans", universe = do.call(c, rmats_diff_res$S3_v_S4$universe))
rmats_ora_S3vS4$GO_BP_ora@result$qvalue <- round(rmats_ora_S3vS4$GO_BP_ora@result$qvalue, 3)
write.table(subset(rmats_ora_F1vP0$GO_BP_ora@result, qvalue < 0.05)[, c("Description", "qvalue")], row.names = F, file = "./results/diffsplice/rmats_F1_v_P0.bp.tsv", quote = F, sep = "\t")

rmats_ora_S4vF3 <- enrich_CP(do.call(c, lapply(rmats_diff_res$S4_v_F3, function(x) {
  return(x$GeneID)
})), "celegans", universe = do.call(c, rmats_diff_res$S4_v_F3$universe))
rmats_ora_S4vF3$GO_BP_ora@result$qvalue <- round(rmats_ora_S4vF3$GO_BP_ora@result$qvalue, 3)
write.table(subset(rmats_ora_F1vP0$GO_BP_ora@result, qvalue < 0.05)[, c("Description", "qvalue")], row.names = F, file = "./results/diffsplice/rmats_F1_v_P0.bp.tsv", quote = F, sep = "\t")



rmats_ora_F1vP0 <- enrich_CP(do.call(c, lapply(rmats_diff_res$F1_v_P0, function(x) {
  return(x$GeneID)
})), "celegans", universe = do.call(c, rmats_diff_res$F1_v_P0$universe))
