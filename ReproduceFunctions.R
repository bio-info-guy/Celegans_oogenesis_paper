...
prepareMonocle <- function(exprs, meta, features, tpm2abs = T, ct2tpm = F, monocle3 = F) {
  ...
  if (monocle3) {
    cds <- new_cell_data_set(
      expression_data = cds@assayData$exprs,
...
geneHeatmap <- function(cds, exprs_mat, tpm = F,
  ...
  heat_obj <- Heatmap(
    matrix = exprs_mat, col = color_pal, name = "Gene Heatmap",
    na_col = "grey",
    color_space = "LAB",
    rect_gp = gpar(col = NA),
    border = T,
...
addTermsHeatmap <- function(cds, genes, meta,
  ...
  genes_clust <- lapply(1:length(hm_order), FUN = function(x) {
    genes[hm_order[[x]]]
  })
  ...
  test_gene_terms <- lapply(gene_terms, FUN = function(sig_terms) {
    if ("GO_BP_ora" %in% names(sig_terms)) {
      if (sum(grepl("death", sig_terms$GO_BP_ora@result$Description[1:5]))) {
        View(sig_terms$GO_BP_ora@result)
      }
  ...
  if (dimension == 2) {
    ...
    if (labeling == TRUE) {
      plot0 <-
        ggplot(test,
          label = T,
          aes(PC1, PC2, color = Stage, label = row.names(test))
        ) +
        geom_text_repel(
          size =
            4
      ...
      plot0 <-
        ggplot(test, label = T, aes(PC1, PC2, color = Stage, shape = Batch)) +
...
monocle_diff <- function(cds, cells = NULL, subtype = NULL, num_cells = 3, min.pct = 0.1, full_formula, reduced_formula, foldChange = "mle", apeglm_method = "nbinomC") {
  ...
  res <- do.call(rbind.data.frame, lapply(diff_test_res, FUN = function(x) {
    x$res
  }))
  ...
  if (!is.null(subtype) & length(subtype) <= 2) {
    ...
    MeanExp <- tapply(colnames(mat), as.character(pData(cds0)[colnames(mat), ]$cellType), function(x) {
      rowMeans(mat[, x])
    })
    ...
    if (foldChange == "vst") { # using vst values to calculate log2FC, not recommended by authors
      ...
      for (l in c("BiocGenerics", "VGAM", "Matrix")) {
...
logfc_shrink <- function(obj, subtype, apeglm_method = "nbinomC") {
  ...
  fit <- apeglm(
    Y = apeglm_mat, x = design, log.lik = logLikNB, param = as.matrix(res[, "disp", drop = F]),
...
plot_genes_diagram <- function(cds, color_by = color_by, single_scale = F, ncols = 5, sep_plots = F, sep_plots_path = NULL, diff_sig = NULL) {
  average_exprs_group <- do.call(rbind, lapply(row.names(fData(cds)), function(g) {
    tapply(row.names(pData(cds)), pData(cds)[, color_by], function(c) {
      mean(log10(t(t(exprs(cds)) / sizeFactors(cds))[g, c] + 1))
    })
  }))
...
plot_cor_heatmap <- function(cor_mat, other_data_name, plot_name) {
  ...
  ggplot(data = melted_corr_mat, aes(x = Var1, y = Var2, fill = Correlation, label = round(Sig_cor, 3))) +
    geom_tile() +
    labs(x = other_data_name, y = "Our Data", fill = "Pearson's\nCorrelation", title = paste("Correlations with ", other_data_name)) +
    scale_fill_gradient2(mid = "#FBFEF9", low = "#0C6291", high = "#A63446", midpoint = (min_cor + max_cor) / 2, limits = c(min_cor, max_cor)) +
    geom_text() +
    theme_classic() +
